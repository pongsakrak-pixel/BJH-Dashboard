...
| **ไฟล์ STATE** | `STATE_V592.md` — **ชื่อไฟล์มี version เสมอ** |
...
# 📅 งานวันที่ 24 ก.ค. 2569 — V579 → V592

## 🏥 Install Base — รื้อใหญ่

### V585–V586 — ตัดของที่ไม่ใช้ + การ์ดตามประเภทสัญญา
**ตัดออก:** ปุ่ม `⬇ Export CSV` (ทุกแท็บ) · คอลัมน์ `เสนอราคา` `วันหมดสัญญา` `เหลือ` · dropdown `สัญญา:` `เสนอราคา:`
→ ทั้งหมดจะไปอยู่ใน **Renewal.html** แทน

**เหตุผลที่ตัด `เสนอราคา`:** 🔴 `con_trigger` **เป็น `'0'` ทั้งระบบ 10,636 เครื่อง** — ไม่มีข้อมูลจริงเลย ทุกแถวขึ้น "รอเสนอราคา" เหมือนกันหมด ถ้าอยากใช้ต้องขอ SmartFlow ส่ง `con_trigger_do_create_quotation` มา

**Model/Brand dropdown กว้างเกิน** — ชื่อรุ่นยาว (`10-550 MyoSure 10-550 Control Unit w`) ดัน select ขยาย → ใส่ `max-width:170px` + `text-overflow:ellipsis`

**เรียง dropdown ประเภทสัญญา** ตามลำดับธุรกิจ (เดิม `.sort()` เรียงตัวอักษรไทยเลยมั่ว):
```
ซื้อขาย → เช่า → เช่าซื้อ → บริการ(รวมอะไหล่) → บริการ(รวม+ยกเว้น)
→ บริการ(ไม่รวม) → บริการ(ไม่รวม+ยกเว้น) → รายครั้ง → บริจาค → ไม่มีสัญญา → ยกเลิก
```
ใช้ `_ctRank()` — ชื่อใหม่ที่ไม่รู้จักต่อท้ายอัตโนมัติ ไม่หาย

### V589 — การ์ด KPI 5 ใบ ไม่ทับซ้อน

| การ์ด | นิยาม | จำนวนจริง |
|---|---|---|
| เครื่องทั้งหมด | — | 10,636 |
| สัญญาซื้อขาย (Active) | ซื้อขาย/เช่า/บริจาค + สัญญายังไม่หมด | 1,076 |
| สัญญาบริการ (Active) | บริการ/รายครั้ง + สัญญายังไม่หมด | 1,059 |
| หมดสัญญาแล้ว | เคยมีสัญญา แต่หมดอายุ ← **กลุ่มที่ตามกลับได้** | 2,814 |
| ไม่มีข้อมูลสัญญา | `ctype` ว่าง / ยกเลิก | 5,687 |

**4 กลุ่มบวกกัน = 10,636 พอดี** · กดการ์ดกรองตารางได้ (กดซ้ำ = ยกเลิก)
**+ จำนวนสัญญา** (unique `CON_NO` เติม 91%) กำกับใต้ตัวเลขเครื่อง

> **บทเรียน:** V587 เคยทำ "Active/หมดแล้ว" เป็นบรรทัดย่อยในการ์ด → **ตัวเลขบวกไม่ลงตัว** (1,217+981 ≠ 2,176 เพราะ 1 สัญญาครอบเครื่องข้ามกลุ่ม) แก้ด้วยการแยก `exp` เป็นกลุ่มของตัวเอง

**`_ibGroupOf()` แยกเป็น 2 ชั้น:**
```js
_ibCtypeOf(x)  → ประเภทดิบ (buy/svc/none) ไม่สนวันหมด
_ibGroupOf(x)  → หมดแล้ว? 'exp' : ประเภท
```

---

## 🔍 Popup รายละเอียดเครื่อง — 6 หมวด + แท็บจริง

| หมวด | เนื้อหา | id |
|---|---|---|
| 1️⃣ ข้อมูลเครื่อง | ลูกค้า · จังหวัด · ผู้แทน · ยี่ห้อ/รุ่น · กลุ่มรุ่น · SN · วันติดตั้ง · อายุ · สถานะ · **เลขที่สัญญา** · ประเภท · วันหมด | `ibd-s1` |
| 2️⃣ ประวัติสัญญา | + **เน้นบรรทัดล่าสุด** 🟢 Active / 🔴 หมดแล้ว | `ibd-s2` |
| 3️⃣ ประวัติอะไหล่ | + **การ์ด ✓ในประกัน vs ✗เบิกซื้อ** + คอลัมน์ **Job SR** | `ibd-s3` |
| 4️⃣ Predict ความเสี่ยง | ฐาน 2 ชั้น (กลุ่ม→รุ่น) · median ราคา · สรุป 5 ชิ้น | `ibd-s4` |
| 5️⃣ แนะนำราคา | ตารางราคาอ้างอิง (เดิม) | `ibd-s5` |
| 6️⃣ ประวัติเสนอราคา | **แยก 2 ตาราง** 📋 QSC · 🔧 QSP | `ibd-s6` |

**V587 แท็บจริง** — กดแล้วโชว์ทีละหมวด (เดิมแค่ scroll) · ปุ่มที่เลือกเน้นตัวหนา+ขอบสี · เปิดมาที่หมวด 1
**V592 ล็อกความสูง** `height:60vh;max-height:60vh` → สลับแท็บไม่เด้งขึ้นลง

### 🔑 QSP ไม่ได้อยู่ใน `_jsupQuo`
```
_jsupQuo = 5,805 ใบ · QH_TYPE='SC' ทั้งหมด · DOC_NUMBER ขึ้นต้น 'QSC' ล้วน
→ ใบเสนอราคาอะไหล่ (QSP) ฝังอยู่ใน _jsupParts.QH_DOC_NUMBER (เติม 57%)
```
V591 จึง **group `partsList` by QN** สร้างตาราง QSP ขึ้นมาเอง · QN ว่าง = เบิกในประกัน ไม่นับเป็นใบเสนอราคา

---

## 📤 V577.1 — Export คอลัมน์ New/Renew

🔑 **field ที่ถูกคือ `r.cnr` (`CON_TYPE_TYPE`) ไม่ใช่ `r.ctn`**

| field | มาจาก | เนื้อหา | ครอบคลุม |
|---|---|---|---|
| `r.ctn` | `CONTRACT_TYPE_NAME` | ชื่อความคุ้มครอง ("สัญญาบริการ (รวมอะไหล่)") | ❌ 0% |
| **`r.cnr`** | **`CON_TYPE_TYPE`** | **NEW / RENEW** | ✅ **92%** |
| `_proMap` | prospect | isNew | 9% |

ผลจริง SC 3,051 ใบ: `RENEW=2299 · NEW=504 · ว่าง=232 · null=16`

---

## 🎨 V571–V576 — Typography หน้า Story

**ขนาดที่ลงตัว:** `STORY OF` 20px · `BUDGET 2026` 24px · `฿250.00M` 28px · `— Annual Target`/`H1`/`H2` 18px · ตัวเลขการ์ด 21px · การ์ดใหญ่ 28px · ยอดหัวกลุ่ม 13px

**V578** `STORY OF 2026` ไล่สีรุ้ง + 📖 โยก 7° รอบละ 2.6s (หยุดตอน Present + `prefers-reduced-motion`)
**V580** ตัด subname (`Ordered + Carry`) + `ของจริง (commit)` → ทุกการ์ดโครงสร้างเหมือนกัน · เปลี่ยนชื่อ `Backlog`→`Sale Order` · `BL Confirm`→`Sales Confirmed` (ตรงกับ Quick Filter)
**V581** ยึดตัวเลขจากบน (`margin-top:2px`) แทน `margin:auto` → ระดับตรงกันทุกใบ

### 🔴 บทเรียน CSS 2 ข้อ

**1. `var(--xxx)` เสมอ ห้าม hardcode สี** — V576 ใส่ `#60a5fa` + gradient ดำ → light mode มองไม่เห็นเลย แก้ด้วย `var(--cyn)` (dark `#0ea5e9` / light `#043d84`)

**2. `flex-wrap` พัง flex column** — V572 ทำ `.flow-card{flex-direction:column}` แล้ว V576 ใส่ `flex-wrap:wrap` → ทุกอย่างเรียงแนวนอน ตัวเลขล้นการ์ด
→ **syntax ผ่าน ≠ layout ถูก** งาน CSS ต้องดู mockup + ตรวจว่ากฎใหม่ไม่ขัดกฎเดิม

---

## 🔧 V582–V584 — สั่ง SP + Install Base filter

- **chip สั่ง SP single-select** — กดซ้ำ = ยกเลิก (เดิม multi)
- **V590 ถอนช่อง search ใน dropdown** 🔴 วิธี `position:absolute` ลอยใต้ `<select multiple>` **ใช้ไม่ได้** — ช่องลอยทับตาราง คุมเปิด/ปิดไม่ได้ placeholder เพี้ยน
  → ถ้าจะทำใหม่ **ต้องสร้าง dropdown เอง ไม่ใช้ `<select>` ของเบราว์เซอร์**

---

# ⏳ งานที่ตกลงแล้ว รอลงโค้ด

## 1. G.Model อยู่ได้หลายกลุ่ม
```js
{ "3Dimensions": "Mammo" }              // เดิม
{ "3Dimensions": ["Mammo","Premium"] }  // ใหม่
```
| เรื่อง | ตกลง |
|---|---|
| หน้า Config | **แบบ A** — คงเดิม (เลือกกลุ่ม → ติ๊กรุ่น) แต่รุ่นที่อยู่กลุ่มอื่นแล้วติ๊กได้ แสดงว่า "อยู่ใน X, Y" |
| Predict ใช้กลุ่มไหน | **แบบ A** — รวมทุกกลุ่มที่รุ่นนั้นอยู่ (ฐานใหญ่สุด) แสดง "Mammo + Premium (18 รุ่น)" |
| dropdown กรอง | เลือกหลายกลุ่มพร้อมกัน → เครื่องโผล่ครั้งเดียว (dedupe) |
| ข้อมูลเดิม 130 รุ่น | แปลง `"A"` → `["A"]` อัตโนมัติตอนโหลด **ไม่ต้องตั้งใหม่** |

**ต้องแตะ:** `Code.js` (section `model_group` รับ array) ⚠️ ต้อง `create-deployment` · filter · popup Config · Predict

**ครอบคลุมปัจจุบัน:** 130 รุ่น · 12 กลุ่ม · 2,461/10,636 เครื่อง (23%) · Install Base มี **816 รุ่น** map แล้ว 127 (16%)

## 2. Predict ความเสี่ยง — logic ใหม่

**แสดง 5 อะไหล่เสี่ยงสุด:**
| คอลัมน์ | คิดยังไง |
|---|---|
| **เสี่ยง %** | เครื่อง (SN) ที่เคยเปลี่ยนอะไหล่นี้ ÷ เครื่องทั้งหมดในกลุ่ม *(เดิมนับ "งาน" ไม่ใช่เครื่อง)* |
| **มักเปลี่ยนปีที่** | median อายุเครื่องตอนเปลี่ยน (โยง `SN` → `installDate`) |
| **เครื่องนี้** | เปลี่ยนไปแล้วกี่ปี / ยังไม่เคย |
| 🔴 **ใกล้ถึงคิว** | อายุเครื่องอยู่ในช่วงที่มักเปลี่ยน (±1 ปี) **และยังไม่เคยเปลี่ยน** |
| **ราคาประมาณ** | median `QH_TOTAL_AMT` (เติม 57%) |

**สรุปล่าง:** "🔴 ใกล้ถึงคิว N ชิ้น · รวม ~฿X" → เอาไปเสนอลูกค้าได้เลย

**กรอง:** ทุกรายการที่ไม่ใช่ `Canceled` (**แบบ A** — รวมทั้งเบิกซื้อและเบิกในประกัน)
**กันฐานน้อย:** < 5 เครื่อง → เตือน "ข้อมูลน้อย" + ไม่ใส่ธง 🔴
**ถ้าข้อมูลย้อนหลังสั้น** → ตัวคูณอายุ fallback เป็น 1.0 อัตโนมัติ ไม่พัง

> ⚠️ **ยังไม่ได้เช็ค:** ข้อมูลอะไหล่ย้อนไปถึงปีไหน — ถ้ามีแค่ 2-3 ปี "มักเปลี่ยนปีที่ 5" จะคำนวณไม่ได้
> รันดูได้: สแกน `ENG_REQ_DATETIME` min/max ใน `_jsupParts`

---

# 📌 ข้อมูลที่ยืนยันแล้ว (24 ก.ค.)

## Install Base — แบ่งตามประเภทสัญญา
```
ไม่มี ctype  = 5,689 (53%)   ← ในนั้นมี hasCon จริงแค่ 5 เครื่อง
สัญญาซื้อขาย = 2,783
สัญญาบริการ  = 2,074
เช่า/เช่าซื้อ = 70
ยกเลิกสัญญา  = 8
...
## Install Base — แบ่งตามอายุ/สถานะ
```
มีสัญญา (hasCon)      = 4,952   · มี CON_NO 4,526 (91%) · สัญญา unique 2,176
หมดใน ≤60 วัน         = 176
หมดใน ≤180 วัน        = 1,149
เพิ่งหมด ≤180 วัน      = 493
หมดเกิน 1 ปี          = 1,756
ไม่มีสัญญา + ≤2 ปี     = 175
ไม่มีสัญญา + ≤5 ปี     = 1,758
ไม่มีสัญญา + >10 ปี    = 1,579
```
> `CON_NO` มี 2 รูปแบบ: `6/2569` (ปีไทย) และ `PO2026060004`

## G.Model
```
130 รุ่น · 12 กลุ่ม · ครอบคลุม 2,461/10,636 เครื่อง (23%)
Install Base มี 816 รุ่น → map แล้ว 127 รุ่น (16%)
```

---

# 🔴 บทเรียนใหญ่ 24 ก.ค. — ABORT ซ้ำ 5 รอบ

**สาเหตุเดียวกันทุกครั้ง:** ผมเดาสถานะจาก **GitHub repo** แล้วสร้าง patch ที่ **รวม version ก่อนหน้าซ้ำ** → `ABORT count=0`

ความจริง: **deploy สำเร็จแล้วแต่ `git push` ไม่สำเร็จ** → repo ตามหลังเครื่อง

## ✅ วิธีแก้ที่ใช้จริง (ตั้งแต่ V591)

**1. ทุกคำสั่งจบด้วยการโชว์เวอร์ชัน**
```powershell
Write-Host ''; Write-Host '--- version ---'; Select-String -Path overrides.html -Pattern "_BUILD_VER='V"
```
→ เห็นทันทีว่าอยู่เวอร์ชันไหน ไม่ต้องถามซ้ำ

**2. ถ้าไม่มั่นใจ ขอ marker count ก่อนสร้าง patch**
```powershell
cd C:\bjh-dashboard; Select-String -Path overrides.html -Pattern "_BUILD_VER='V"; foreach($k in @('V585','V586','V587','_spQuo','jobSr')){ $n=(Select-String -Path SalesProspect.html -Pattern ([regex]::Escape($k)) -AllMatches | Measure-Object).Count; Write-Host "$k = $n" }
```

**3. ⚠️ `Start-Transcript` ไม่น่าเชื่อถือ** — บางครั้งไฟล์ว่าง/เขียนไม่ทัน
→ ให้ผลขึ้นหน้าจอตรงๆ แล้ว Eak ก็อปมาวาง ดีกว่า

**4. ⚠️ ระวังนับ marker ผิด** — `_spQuo` ไปเจอ `sp_spquote` ทำให้นับได้ 2 ทั้งที่ไม่มีจริง
→ ใช้ `\b` หรือดูบรรทัดจริงประกอบ

---

# 💻 หมายเหตุการทำงาน

**Eak ทำงานบนมือถือได้** — ดู dashboard · ถ่ายรูป · คุยแผน · ดู mockup
**ทำไม่ได้บนมือถือ:** รันคำสั่ง PowerShell / clasp / git
→ ถ้า Eak อยู่บนมือถือ ให้คุยแผนให้จบ แล้วเก็บคำสั่งไว้รันตอนกลับหน้าคอม
