...
function _getConfig_() {
  try {
    ...
    // [V654] Clear รายการ Critical Down — { SR: '2026-08-03 ชื่อคนกด' }
    //   V601 เขียนฝั่ง client ไว้อย่างเดียว — backend ไม่มี section นี้ กดแล้วจึงไม่จำ
    if (ckv.cd_cleared && Object.keys(ckv.cd_cleared).length > 0) {
      config.cd_cleared = {};
      Object.keys(ckv.cd_cleared).forEach(function(k){
        var v = String(ckv.cd_cleared[k]||'').trim();
        if (v) config.cd_cleared[k] = v;
      });
    }
